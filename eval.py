...
import utils
from attacker import SRGAttacker, ImageAttacker, TextAttacker
from dataset import paired_dataset


def retrieval_eval(model, ref_model, target_models, target_ref_models, target_transforms,
                   data_loader, tokenizer, target_tokenizers, device, config):
    model.float()
    model.eval()
    ref_model.eval()

    for t_model in target_models:
        if hasattr(t_model, 'float'):
            t_model.float()
        t_model.eval()

    for t_ref_model in target_ref_models:
        if t_ref_model is not None:
            t_ref_model.eval()
    ...
    target_features = {}
    target_names = ['TCL', 'CLIP_ViT', 'CLIP_CNN']

    for target_idx, target_name in enumerate(target_names):
        if target_name == 'TCL':
            target_features[target_name] = {
            ...
            clip_output_dim = target_models[target_idx].visual.output_dim
            target_features[target_name] = {
                'image_feats': torch.zeros(num_image, clip_output_dim),
                'text_feats': torch.zeros(num_text, clip_output_dim)
            }
    ...
    for batch_idx, (images, texts_group, images_ids, text_ids_groups) in enumerate(data_loader):
        ...
        with torch.no_grad():
            ...
            adv_texts_input = tokenizer(adv_texts, padding='max_length', truncation=True,
            ...
            for target_idx, target_name in enumerate(target_names):
                if target_name == 'TCL':
                    ...
                    for itm in adv_images:
                        t_adv_img_list.append(target_transforms[target_idx](itm))
                    t_adv_imgs = torch.stack(t_adv_img_list, 0).to(device)

                    t_adv_images_norm = images_normalize(t_adv_imgs)
                    t_adv_texts_input = target_tokenizers[target_idx](adv_texts, padding='max_length',
                                                                      truncation=True, max_length=30,
                                                                      return_tensors="pt").to(device)
                    t_output_img = target_models[target_idx].inference_image(t_adv_images_norm)
                    t_output_txt = target_models[target_idx].inference_text(t_adv_texts_input)

                    target_features[target_name]['image_feats'][images_ids] = t_output_img['image_feat'].cpu().detach()
                    target_features[target_name]['image_embeds'][images_ids] = t_output_img[
                        'image_embed'].cpu().detach()
                    target_features[target_name]['text_feats'][texts_ids] = t_output_txt['text_feat'].cpu().detach()
                    target_features[target_name]['text_embeds'][texts_ids] = t_output_txt['text_embed'].cpu().detach()
                    target_features[target_name]['text_atts'][
                        texts_ids] = t_adv_texts_input.attention_mask.cpu().detach()
                    ...
                    t_adv_images_norm = images_normalize(t_adv_imgs)
                    output = target_models[target_idx].inference(t_adv_images_norm, adv_texts)
                    target_features[target_name]['image_feats'][images_ids] = output[
                        'image_feat'].cpu().float().detach()
                    target_features[target_name]['text_feats'][texts_ids] = output['text_feat'].cpu().float().detach()

    s_score_matrix_i2t, s_score_matrix_t2i = retrieval_score(model, s_image_feats, s_image_embeds,
                                                             s_text_feats, s_text_embeds, s_text_atts,
                                                             num_image, num_text, device=device)

    target_scores = {}
    for target_idx, target_name in enumerate(target_names):
        if target_name == 'TCL':
            t_score_matrix_i2t, t_score_matrix_t2i = retrieval_score(
                target_models[target_idx],
                target_features[target_name]['image_feats'],
                target_features[target_name]['image_embeds'],
                target_features[target_name]['text_feats'],
                target_features[target_name]['text_embeds'],
                target_features[target_name]['text_atts'],
                num_image, num_text, device=device
            )
            target_scores[target_name] = {
                'i2t': t_score_matrix_i2t.cpu().numpy(),
                't2i': t_score_matrix_t2i.cpu().numpy()
            }
        else:
            t_sims_matrix = target_features[target_name]['image_feats'] @ target_features[target_name]['text_feats'].t()
            target_scores[target_name] = {
                'i2t': t_sims_matrix.cpu().numpy(),
                't2i': t_sims_matrix.t().cpu().numpy()
            }
...
@torch.no_grad()
def retrieval_score(model, image_feats, image_embeds, text_feats, text_embeds, text_atts,
                    num_image, num_text, device=None):
    if device is None:
...
def eval_asr(model, ref_model, tokenizer, target_models, target_ref_models,
             target_tokenizers, target_transforms, data_loader, device, args, config):
    model = model.to(device)
    ref_model = ref_model.to(device)

    for i, t_model in enumerate(target_models):
        target_models[i] = t_model.to(device)
        if target_ref_models[i] is not None:
            target_ref_models[i] = target_ref_models[i].to(device)

    start_time = time.time()

    score_i2t, score_t2i, target_scores = retrieval_eval(
        model, ref_model, target_models, target_ref_models, target_transforms,
        data_loader, tokenizer, target_tokenizers, device, config
    )
    ...
    # source model result
    albef_result = itm_eval(score_i2t, score_t2i, data_loader.dataset.img2txt,
                            data_loader.dataset.txt2img, 'ALBEF')
    final_result += "=" * 50 + "\n"
    final_result += "Source Model (ALBEF) Performance:\n"
    final_result += "=" * 50 + "\n"
    for key, val in albef_result.items():
        final_result += f"{key}: {val}\n"
    final_result += "\n"

    # target models results
    target_names = ['TCL', 'CLIP_ViT', 'CLIP_CNN']
    for target_name in target_names:
        if target_name in target_scores:
            t_result = itm_eval(target_scores[target_name]['i2t'],
                                target_scores[target_name]['t2i'],
                                data_loader.dataset.img2txt,
                                data_loader.dataset.txt2img,
                                target_name)
            final_result += "=" * 50 + "\n"
            final_result += f"Target Model ({target_name}) Performance:\n"
            final_result += "=" * 50 + "\n"
    ...
    final_result += "=" * 50 + "\n"
    final_result += f"Evaluation Total Time: {total_time_str}\n"
    final_result += "=" * 50 + "\n"
...
def main(args, config):
    ...
    model, ref_model, tokenizer = load_model(args.source_model, args.source_ckpt,
                                             args.source_text_encoder, device)
    ...
    # TCL
    tcl_model, tcl_ref_model, tcl_tokenizer = load_model('TCL', args.tcl_ckpt,
                                                         args.target_text_encoder, device)
    target_models.append(tcl_model)
    target_ref_models.append(tcl_ref_model)
    target_tokenizers.append(tcl_tokenizer)
    target_transforms.append(transforms.Compose([
    ...
    # CLIP-ViT
    clip_vit_model, clip_vit_ref_model, clip_vit_tokenizer = load_model('ViT-B/16', None,
                                                                        args.target_text_encoder, device)
    target_models.append(clip_vit_model)
    target_ref_models.append(clip_vit_ref_model)
    target_tokenizers.append(clip_vit_tokenizer)
    n_px_vit = clip_vit_model.visual.input_resolution
    target_transforms.append(transforms.Compose([
        transforms.Resize(n_px_vit, interpolation=transforms.InterpolationMode.BICUBIC),
        transforms.CenterCrop(n_px_vit),
    ]))

    # CLIP-CNN
    clip_cnn_model, clip_cnn_ref_model, clip_cnn_tokenizer = load_model('RN101', None,
                                                                        args.target_text_encoder, device)
    target_models.append(clip_cnn_model)
    target_ref_models.append(clip_cnn_ref_model)
    target_tokenizers.append(clip_cnn_tokenizer)
    n_px_cnn = clip_cnn_model.visual.input_resolution
    target_transforms.append(transforms.Compose([
        transforms.Resize(n_px_cnn, interpolation=transforms.InterpolationMode.BICUBIC),
        transforms.CenterCrop(n_px_cnn),
    ]))
    ...
    eval_asr(model, ref_model, tokenizer, target_models, target_ref_models,
             target_tokenizers, target_transforms, test_loader, device, args, config)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source_ckpt', default='./checkpoint/albef_retrieval_flickr.pth', type=str)

    parser.add_argument('--target_text_encoder', default='bert-base-uncased', type=str)
    parser.add_argument('--tcl_ckpt', default='./checkpoint/tcl_retrieval_flickr.pth', type=str)
